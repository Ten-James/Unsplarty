...
const Lobby = () => {
  ...
  return (
    <Base title="Lobby">
      ...
      {amIMaster ? (
        <Button fullWidth variant="outlined" onClick={() => setGameState('gameSelect')}>
          Start
        </Button>
      ) : players ? (
      ...
      <Stack direction="row" justifyContent="center" alignItems="center" spacing={2}>
        <FormControlLabel control={<MaterialUISwitch sx={{ m: 1 }} checked={theme} onChange={e => changeTheme(e.target.checked)} />} label="Theme" />
      </Stack>
    </Base>
  );
};
