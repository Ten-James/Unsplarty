...
function App() {
  ...
  return (
    <div className="App">
      ...
      {gameState === 'lobby' && <Lobby players={players} name={name} setName={setName} isAdmin={amIMaster} />}
      {gameState === 'gameSelect' && <GameSelect players={players} currentGame={currentGame} onGameSelect={(game) => write('currentGame', game)} isAdmin={amIMaster} />}
      {gameState === 'choosing' &&  !playerOrder && <h1>Loading...</h1>}
      {gameState === 'choosing' && playerOrder && <ThemeSelect amIChooser={playerOrder[currentPlayer] === name} />}
      {gameState === 'playing' && (!image || !fakeImage)&& <h1>Loading...</h1>}
      {gameState === 'playing' && image && fakeImage && <Game imageUrls={[image, ...fakeImage]} amIChooser={playerOrder[currentPlayer] === name} />}
    </div>
  );
};
